        ...
        for (const st of aiSettings) {
            ...
            for (const sender_number in groups) {
                ...
                if (lastMsg.role === 'assistant') {
                    ...
                    if (userMsgIndex > 1) {
                        debugLog.push({ sender: sender_number, skip: 'Already sent recovery message' });
                        continue;
                    }
                    ...
                    if (diffMins >= recoveryMins && diffMins <= 120) {
                        ...
                        if (isQuestionPattern) {
                            ...
                            const prompt = `Você é o assistente da clínica. O cliente abaixo parou de responder há ${diffMins} minutos.
Sua missão: Envie UMA ÚNICA mensagem MUITO CURTA (máximo de 12 palavras) para reativar a conversa e ajudar o cliente a continuar.
REGRAS OBRIGATÓRIAS:
- NÃO RE-CITE o nome do cliente para não parecer robótico (a menos que extremamente necessário).
- ESCREVA TUDO EM APENAS 1 LINHA. SEM quebras de linha ou múltiplos parágrafos de forma alguma.
- Seja amigável, casual e super direto. (Exemplo de tom: "Oi! Ficou com alguma dúvida?" ou "Ainda quer continuar o agendamento?")
Aqui está o final da conversa interrompida:
Cliente: "${prevUserMsg.content}"
Sua última fala: "${lastMsg.content}"

Vá direto ao ponto. Apenas retorne o texto final que será enviado no WhatsApp dele.`;
                            ...
                            try {
                                ...
                                let recoveryTxt = aiRes.choices[0].message.content?.trim() || "";
                                
                                // --- SANITIZE HALLUCINATIONS FROM WEAK/FREE OPENROUTER MODELS ---
                                const arrayMatch = recoveryTxt.match(/^\[\s*\{\s*['"]type['"]\s*:\s*['"]text['"]\s*,\s*['"]data['"]\s*:\s*\[\s*\{\s*['"]text['"]\s*:\s*['"]([\s\S]*?)['"]\s*\}\s*\]\s*\}\s*\]$/);
                                if (arrayMatch && arrayMatch[1]) {
                                    recoveryTxt = arrayMatch[1];
                                    try { recoveryTxt = JSON.parse(`"${recoveryTxt}"`); } catch(e){}
                                }

                                // Validate Everything: Remove spurious outer quotes generated by AI
                                recoveryTxt = recoveryTxt.replace(/^["']|["']$/g, '').trim();
